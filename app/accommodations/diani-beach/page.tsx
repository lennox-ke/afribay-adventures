...
export default function DianiBeachAccommodationsPage() {
  const [selectedType, setSelectedType] = useState<(typeof accommodationTypes)[number]>("all")

  const dianiAccommodations = accommodations.filter((acc) => acc.destinationId === "diani-beach")
  const filteredAccommodations =
    selectedType === "all"
      ? dianiAccommodations
      : dianiAccommodations.filter((acc) => acc.type === selectedType)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <Image
            src="https://afribay.vercel.app/diani.jpg"
            alt="Diani Beach Accommodations"
            fill
        ...
        <div className="relative z-10 text-center text-white max-w-4xl mx-auto px-4">
          <h1 className="font-serif text-5xl md:text-6xl font-bold mb-6">Diani Beach Accommodations</h1>
          <p className="text-xl md:text-2xl font-light leading-relaxed">
            Relax on pristine white sand beaches with luxury beachfront resorts
          </p>
        </div>
      </section>
            ...
            {filteredAccommodations.length === 0 && (
              <div className="col-span-full text-center py-12">
                <p className="text-gray-600 text-lg">
                  No accommodations found for Diani Beach. Please check back soon!
                </p>
              </div>
            )}
          </div>
      ...
      <section className="py-16 bg-afribay-dark-green text-white">
        <div className="max-w-4xl mx-auto text-center px-4">
          <h2 className="font-serif text-3xl md:text-4xl font-bold mb-6">Your Beach Paradise Awaits</h2>
          <p className="text-xl mb-8 opacity-90">
            Discover pristine beaches, coral reefs, and luxury accommodations on Kenya's stunning coast
          </p>
