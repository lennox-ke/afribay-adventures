...
import { Clock, Users, MapPin, Star, Globe, ArrowUpDown, Heart } from "lucide-react"
...
/* ---------- currency support ----------
   Same pattern as the luxury and package detail pages — international
   travelers want to see cost in their own currency. Rates are approximate
   USD conversions, clearly labeled. */
const CURRENCIES = [
...
export default function MidRangePackagesPage() {
  const midRangePackages = getPackagesByCategory("mid-range")
  ...
  const destinations = useMemo(() => {
    ...
    midRangePackages.forEach((pkg) => pkg.destinations.forEach((d: string) => set.add(d)))
    return Array.from(set)
  }, [midRangePackages])

  const visiblePackages = useMemo(() => {
    let list = [...midRangePackages]
    ...
    return list
  }, [midRangePackages, sortBy, destinationFilter])
  ...
  return (
    <PageTransition type="elegant" duration={600}>
      <main className="min-h-screen bg-[#F2EFED]">
        ...
        {/* ── HERO ───────────────────────────────────────────────────────── */}
        <section className="relative pt-28 pb-20 bg-gradient-to-r from-[#7FB5B5] to-[#E8A17D] overflow-hidden">
          <div
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="inline-flex items-center gap-2 mb-5 rounded-full bg-white/15 px-4 py-1.5">
            ...
            <p className="text-xl text-white/90 max-w-3xl mx-auto leading-relaxed">
              Perfect balance of comfort and adventure with our mid-range packages
              offering excellent value, quality accommodations, and authentic
                ...
                <button
                  ...
                  className={`shrink-0 px-4 py-1.5 text-xs font-semibold rounded-full border transition-colors duration-200 ${
                    destinationFilter === "all"
                      ...
                      : "bg-white text-stone-600 border-stone-200 hover:border-[#E8A17D]"
                  }`}
                >
                  All Destinations
                </button>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <div className="flex items-center gap-1.5 text-stone-500">
                  ...
                  <select
                    ...
                    className="text-xs font-semibold bg-white border border-stone-200 rounded-full pl-3 pr-2 py-1.5 text-[#2F3B2F] focus:outline-none focus:ring-2 focus:ring-[#E8A17D]/40"
                  >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {visiblePackages.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-stone-500">No mid-range packages match this destination yet.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {visiblePackages.map((pkg) => {
                  const priceWithTransport = totalPrice(pkg)
                    ...
                    >
                      ...
                      <div className="relative h-64 overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 flex items-center gap-2">
                          <Badge className="bg-[#E8A17D] text-white text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wide">
                            Mid-Range
                          </Badge>
                          {pkg.originalPrice && (
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-end justify-between gap-3">
                          <div>
                            ...
                            <div className="flex items-baseline gap-2">
                              <span className="text-2xl font-bold text-[#2F3B2F]">
                                {convertAmount(priceWithTransport, currency)}
                              </span>
                              {pkg.originalPrice && (
                            ...
                            <p className="text-xs text-stone-400 mt-0.5">
                              {currency !== "USD" ? `≈ ${formatPrice(priceWithTransport)} USD · ` : ""}
                          ...
                          <Button
                            asChild
                            className="bg-[#E8A17D] hover:bg-[#d4906c] text-white text-sm rounded-full px-6 py-2.5 font-semibold transition-colors duration-300 shrink-0"
                          >
